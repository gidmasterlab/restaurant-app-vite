...
describe('Verify card detail elements', () => {
	...
	it('should return the expected category', () => {
		expect(wrapper.find('.category').find('.text').text()).toEqual(PRODUCT.category)
		expect(wrapper.find('.category').find('.icon').attributes()['src']).toContain(PRODUCT.category)
	})

	it('should return the expected review', () => {
		const review = formatReview(PRODUCT.totalRating, PRODUCT.totalReviews)
		expect(wrapper.find('.review').find('.text').text()).toContain(review)
		expect(wrapper.find('.review').find('.text').text()).toContain(`(${PRODUCT.totalReviews})`)
	})
